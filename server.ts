...
app.post('/api/chat', async (req, res) => {
  ...
  if (!isLegal) {
    // 1. Identity questions like "هل انت انسان"
    if (
      /^(هل انت انسان|هل أنت إنسان|انت انسان|أنت إنسان|هل انت بشر|هل أنت بشر|هل انت روبوت|هل أنت روبوت|هل انت ai|هل انت ذكاء اصطناعي)\b/i.test(
        trimmed.replace(/[!؟?.,\s]+/g, ' ')
      )
    ) {
      return res.json({
        reply: `لا، أنا لست إنساناً. أنا مساعد ذكاء اصطناعي ذكي مصمم لمساعدتك والإجابة على جميع أسئلتك واستفساراتك العامة والمتخصصة بكل دقة وسهولة.`,
        isFastReply: true,
      });
    }
    ...
    if (
      /^(مين انت|من انت|من أنت|ما اسمك|ما هو اسمك|عرف عن نفسك|عرفني بنفسك|شو اسمك|شو وظيفتك|ما وظيفتك)\b/i.test(
        trimmed.replace(/[!؟?.,\s]+/g, ' ')
      )
    ) {
      return res.json({
        reply: `أنا «سَنَد»، مساعد ذكاء اصطناعي شامل، وخبير في القوانين والضرائب والجمارك الفلسطينية، بالإضافة إلى قدرتي على الإجابة عن كافة الأسئلة العامة والمعرفية ومساعدتك في أي موضوع.`,
        isFastReply: true,
      });
    }
    ...
    // 3. Greetings
    if (
      /^(سلام|السلام عليكم|سلام عليكم|مرحبا|أهلا|اهلا|مرحباً|صباح الخير|مساء الخير|هاي|hello|hi)\b/i.test(
        trimmed.replace(/[!؟?.,\s]+/g, ' ')
      )
    ) {
      return res.json({
        reply: `وعليكم السلام ورحمة الله وبركاته! أهلاً وسهلاً بك. أنا مساعدك الذكي «سَنَد»، كيف يمكنني مساعدتك اليوم؟`,
        isFastReply: true,
      });
    }
    ...
    if (/^(عامل ايه|عامل إيه|كيفك|كيف حالك|ازيك|إزيك|شخبارك|أخبارك|شو أخبارك|شو اخبارك)/i.test(trimmed)) {
      return res.json({
        reply: `الحمد لله بألف خير ونعمة، شكراً لسؤالك ولطفك! أنا هنا في خدمتك، تفضل بأي استفسار أو موضوع تريد التحدث فيه.`,
        isFastReply: true,
      });
    }

    // 5. Thanks
    if (/^(شكرا|شكراً|تسلم|مشكور|الله يعطيك العافية|يعطيك العافية|يسلمو|بارك الله فيك)/i.test(trimmed)) {
      return res.json({
        reply: `العفو، على الرحب والسعة دائماً! أنا في خدمتك في أي وقت لأي سؤال أو استفسار.`,
        isFastReply: true,
      });
    }
  }
  ...
  const systemInstruction = `أنت "سَنَد"، المساعد والذكاء الاصطناعي الشامل والمستشار القانوني والضريبي والجمركي في دولة فلسطين.

قواعد واستراتيجية الإجابة الصارمة:
1. **الأسئلة العامة والمعرفة العامة وأي موضوع خارج القوانين:**
   - أجب على أي سؤال أو موضوع عام يسألك عنه المستخدم (سواء كان عن الدين الإسلامي، الشخصيات مثل محمد صلاح، العلوم، الرياضة، التاريخ، الثقافة، التكنولوجيا، أو الدردشة اليومية).
   - إذا سألك المستخدم "هل أنت إنسان؟": أجب مباشرة وبلباقة (مثال: "لا، أنا لست إنساناً، أنا مساعد ذكاء اصطناعي ذكي...").
   - **ممنوع بتاتاً** إدراج أي مواد أو نصوص أو مراسيم قانونية في الإجابات العامة غير القانونية! الإجابة تكون ذكية ومباشرة وودية.

2. **الاستشارات والأسئلة القانونية والضريبية والجمركية (فلسطين):**
   عندما يسألك المستخدم عن أي قانون أو ضريبة أو جمرك، يجب أن تكون إجابتك **مبسطة جداً، واضحة ومباشرة دون تعقيد أو إطالة غير مفيدة**، وبشكل محدد ومنظم:
   
   📌 **1. الإجابة المبسطة المباشرة:**
   - شرح عملي مبسط جداً ومباشر لما يسأل عنه المستخدم دون تعقيد.

   ⚖️ **2. السند القانوني والمادة المحددة:**
   - **اسم التشريع:** (اسم القانون أو القرار بقانون بدقة).
   - **توقيت وسنة الإصدار:** (تاريخ أو سنة الصدور والنفاذ).
   - **رقم المادة/الفقرة المحددة:** (المادة رقم ... فقرة ... فقط).
   - **مضمون المادة باختصار:** اذكر نص البند أو المادة المعنية فقط بدقة ودون سرد نصوص القانون بالكامل أو إغراق المستخدم بالمواد غير المطلوبة.
...
function isLegalTaxCustomsQuery(query: string): boolean {
  ...
  // 1. Explicit conversational, personal, greeting, or philosophical/general queries
  if (
    /^(هل انت انسان|هل أنت إنسان|انت انسان|أنت إنسان|هل انت بشر|هل أنت بشر|هل انت روبوت|هل أنت روبوت|هل انت ai|هل انت ذكاء اصطناعي|من انت|مين انت|من أنت|ما اسمك|ما هو اسمك|عرفني بنفسك|عرف عن نفسك|شو اسمك|شو وظيفتك|ما وظيفتك|كيف حالك|كيفك|عامل ايه|ازيك|صباح الخير|مساء الخير|سلام|مرحبا|هاي|hello|hi)\b/i.test(
      q.replace(/[!؟?.,\s]+/g, ' ')
    )
  ) {
    return false;
  ...
  // 2. Clear non-legal general knowledge topics (religion, sport, culture, history, celebrities, science, coding, general definitions)
  if (
    /(محمد صلاح|ميسي|رونالدو|كرة القدم|الرياضة|الدين الإسلامي|دين الاسلام|الإسلام|الاسلام|القرآن|الحديث|الصلاة|الصيام|الحج|الزكاة|النبي|الرسول|الفيزياء|الكيمياء|الطب|الطقس|فلسطين والتاريخ|الجغرافيا|الفلسفة|البرمجة|الرياضيات|معنى كلمة|قصة|نكتة|شعر|الذكاء الاصطناعي|تاريخ|عاصمة|من هو|من هي|ما هو|ما هي|ماذا تعرف عن)/i.test(
      q
    ) &&
    !/(قانون|ضريبة|ضرائب|جمارك|جمرك|رسم جمركي|رسوم جمركية|مرسوم رئاسي|قرار بقانون|مادة رقم|سجل تجاري|مقاصة|إعفاء ضريبي)/i.test(q)
  ) {
    return false;
  }

  // 3. Strict legal keywords matcher for tax, customs, laws, and regulations
  const legalTermsRegex = /(قانون|قوانين|تشريع|تشريعات|مادة|مواد|مرسوم|قرار بقانون|لائحة|لوائح|ضريبة|ضرائب|ضريبي|ضريبية|جمارك|جمرك|جمركي|جمركية|بيان جمركي|رسوم جمركية|تعرفة جمركية|طرد بريدي|إعفاء ضريبي|إعفاءات|دخل كلي|ضريبة دخل|قيمة مضافة|مكوس|غرامة تأخير|عقوبة|محكمة الصلح|وزارة المالية|دائرة الجمارك|مكافحة غسل الأموال|فحص ضريبي|تهرب ضريبي|سجل تجاري|فاتورة ضريبية|مقاصة)/i;
