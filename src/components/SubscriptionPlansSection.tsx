...
export const SubscriptionPlansSection: React.FC<SubscriptionPlansSectionProps> = ({
  ...
  const [plans, setPlans] = useState<SubscriptionPlan[]>(initialPlans || DEFAULT_FALLBACK_PLANS);
  ...
  useEffect(() => {
    ...
    async function loadPlansAndContact() {
      try {
        ...
        if (plansRes.ok) {
          ...
          if (data.plans && Array.isArray(data.plans) && data.plans.length > 0) {
            if (isMounted) setPlans(data.plans);
          } else {
            // Check direct firestore
            const cloudPlans = await directFetchSubscriptionPlansFromFirestore();
            if (cloudPlans && cloudPlans.length > 0 && isMounted) {
              setPlans(cloudPlans.filter(p => p.isActive !== false));
            }
          }
        } else {
          const cloudPlans = await directFetchSubscriptionPlansFromFirestore();
      ...
      // Fetch contact info to obtain accurate WhatsApp phone number
      try {
